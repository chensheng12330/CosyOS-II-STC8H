 ...
 * @file     test_xwtb_sem.c
 * @brief    计数信号量测试
 * @detail   通过计数信号量的获取/给予，实现中断对任务、任务对任务的行为同步（事件同步）。
 * @service  uTakeSem(sem, tick); 任务中获取计数信号量
             uGiveSem(sem);       任务中给予计数信号量
             iGiveSem(sem);       中断中给予计数信号量
 * @author   迟凯峰
 * @version  V1.1.0
 * @date     2024.06.25
 ******************************************************************************/
...
#include "..\Test\test.h"
#if TEST_OBJECT == TEST_XWTB_SEM

/* 创建计数信号量，初值为0，最大值为65535 */
uCreateSem(sem_1, 0, 65535);
...
void start_hook(void)
{
	...
	printf("\xBC\xC6\xCA\xFD\xD0\xC5\xBA\xC5\xC1\xBF\xB2\xE2\xCA\xD4\r\n");
	/* 初始化定时器A */
	TEST_TIMA_INIT;
}
